...
export function AlexUI() {
  ...
  return (
    <div className="pointer-events-none absolute bottom-12 left-12 z-20 flex max-w-2xl flex-col gap-1.5 font-sans text-[15px] leading-relaxed text-neutral-500">
      <div className="animate-fade-in text-neutral-500">
        Durum: <span className="text-neutral-400">{stateText}...</span>
      </div>
      
      {lastUser && (
        <div className="animate-fade-in text-neutral-500">
          Kullanıcı: &quot;<span className="text-neutral-300">{lastUser}</span>&quot;
        </div>
      )}
      
      {lastAssistant && (
        <div className="animate-fade-in text-neutral-500">
          Cevap: &quot;<span className="text-neutral-300">{lastAssistant}</span>&quot;
        </div>
      )}
    </div>
  );
